...
var Printer = function (port, opts) {
  ...
  this.port = port;
  this.baud = 19200;
  this.logger = opts.logger || { info: function () {}, debug: function () {} };
...
Printer.prototype.print = function (doc) {
  ...
  self.setup()
      .then(
        function () {
          self.logger.debug("setup done, calling writeSerial", doc.length);
...
Printer.prototype.writeSerial = function (doc) {
  var self = this;
  self.logger.debug('serialport.write', doc.length, self.serialport);
